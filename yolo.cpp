...
void YOLO::preForward(std::vector<cv::Mat>& images) {
  ...
  for (size_t i = 0; i < images.size(); i++) {
    ...
    bm_image_create(bm_handle_, images[i].rows, images[i].cols,
             FORMAT_BGR_PACKED, DATA_TYPE_EXT_1N_BYTE, &bmimg, NULL);
    bm_image_copy_host_to_device(bmimg, (void**)&images[i].data);
...
detection* YOLO::get_detections(
  ...
  for (unsigned int i = 0; i < blobs.size(); ++i) {
    layer l_params = make_yolo_layer(1, fm_size_[2 * i],
                    fm_size_[2 * i + 1], anchor_num_, anchor_num_ * 3, classes);
...
std::vector<std::vector<yolov3_DetectRect> > YOLO::postForward() {
  ...
  for (int i = 0; i < batch_size_; i++) {
    ...
    blobs.push_back(reinterpret_cast<float*>(
                            outputs_[0]) + output_sizes_[0] * i);
    blobs.push_back(reinterpret_cast<float*>(
                            outputs_[1]) + output_sizes_[1] * i);
    blobs.push_back(reinterpret_cast<float*>(
                            outputs_[2]) + output_sizes_[2] * i);
